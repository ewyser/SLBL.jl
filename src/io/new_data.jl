...
Constructs a `Data` object from a given `AbstractSource` (such as a `File`). This function extracts the file extension, determines the appropriate format (e.g., raster or vector), and initializes a `Data` struct with the provided label and default attributes.

# Arguments
- `source::AbstractSource`: The data source to wrap in a `Data` object.
